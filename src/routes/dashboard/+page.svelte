            ...
            {#each models as model}
                <div class="card">
                    <div class="card-content description">
                        <h3>{model.name}</h3>
                        <p><strong style="font-size: 25px;">Category:</strong> {model.category}</p>
                        <p><strong style="font-size: 25px;">Subtitle:</strong> {model.subtitle}</p>
                        <ul>
                            {#each model.sections || [] as section}
                                <li><strong style="font-size: 25px;">{section.name}:</strong></li>
                                {#each section.entries as entry}
                                    <li><strong style="font-size: 17px;">{entry.key}:</strong> {entry.value}</li>
                                {/each}
                            {/each}
                        </ul>
                        <p><strong style="font-size: 25px;">Bottom Text:</strong> {model.bottom_text}</p>
